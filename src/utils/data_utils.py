...
# prepare dataset
def split_dataset(database: dict, test_size: float=0.2, verbose: bool=True, rng: int=456) -> Any:
    ...
    train_ind, test_ind = train_test_split(all_indices, test_size=test_size, random_state = rng)
...
def prepare_local_predict_dataset(
    ...
    for i in range(nData):
        ...
        for j in range(x.shape[-1]):
            spline_x_ij = interp1d(np.arange(t.size), x[i,:,j], kind='cubic', fill_value=1e-6, bounds_error=False)
...
# prepare test data
def prepare_test_data(u_data: np.ndarray, y_data: np.ndarray, G_data: np.ndarray, nLocs: int=300) -> list:
...
def prepare_test_data_HF(u_data: np.ndarray, y_data: np.ndarray, G_data: np.ndarray, nLocs: int, scale_mode: str, datasetLF: Any, modelLF:Any) -> list:
    u = copy.deepcopy(u_data)
    y = copy.deepcopy(y_data)
    G = copy.deepcopy(G_data)
    ...
    if scale_mode == "normalize":
        G_LF = unnormalize(GG.detach().cpu().numpy(), datasetLF.dataG_mean, datasetLF.dataG_std).flatten()
       
    elif scale_mode == "min-max":
        G_LF= unminmaxscale(GG.detach().cpu().numpy(), datasetLF.dataG_min, datasetLF.dataG_max).flatten()
