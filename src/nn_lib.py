...
class Customize_Dataset(Dataset):
    def __init__(
        ...
        idxs_select = (
            ...
            & (np.isnan(y).sum(axis=1) == 0)
            & (np.isnan(u).sum(axis=1) == 0)
        )
        ...
        if self.search_random:
            ...
            offset = self.offset
            local_idxs[:, :, 0] = (
                offset + local_idxs[:, :, 0] * (t.size - offset - 2 * search_len)
            ).astype(int)
            ...
            offset = self.offset
            idx_end = 1000 - 2 * search_len - offset
            local_idxs[:, :, 0] = (
                (offset + np.linspace(1, idx_end, search_num))
                .reshape(-1, 1)
                .astype(int)
            )
        ...
        print(
            f"Data memory: {(self.data.element_size() * self.data.nelement() + self.metadata.element_size() * self.metadata.nelement()) / 1024 ** 2:.2f} MB"
        )
...
def draw_loss(
    fig_train, epochs_tonow, loss_epoch, accuracy_epoch, accuracy_threshold=0.2
):
    fig_train.clear()
...
def draw_valid(fig_valid, pred_list, y_list, accuracy_threshold=0.2):
    fig_valid.clear()
