...
class LSTM_DeepONet(nn.Module):
    ...
    def forward_default(self, x, metadata, mask):
        ...
        mask_len = (
            mask.sum(axis=-1).type(torch.int64).cpu() if mask is not None else None
        )
        latent_x_packed = (
...
def train_loop(
    dataloader, model, loss_fn, optimizer, input_transform=None, target_transform=None
):
    ...
    train_accuracy = (
        (pred_list > y_list * (1 - accuracy_threshold))
        * (pred_list < y_list * (1 + accuracy_threshold))
    ).sum()
...
if __name__ == "__main__":
    ...
    train_dataset = Customize_Dataset(
        filepath="archive/data/pendulum_u_random_init_a_001.pkl",
        search_len=10,
        search_num=20,
        use_padding=True,
        search_random=True,
    ...
    train_dataloader = DataLoader(
        train_dataset, batch_size=500, drop_last=True, shuffle=True
    )
    valid_dataloader = DataLoader(
        valid_dataset, batch_size=500, drop_last=True, shuffle=True
    )
    ...
    for epoch in range(epoch_num):
        ...
        model.train()
        train_output, train_statistics = train_loop(
            train_dataloader,
            model,
            loss_fn,
            optimizer,
        ...
        loss_train, accuracy_train, r2_train = (
            train_statistics[0],
            train_statistics[1],
            train_statistics[2],
        )
        ...
        model.eval()
        valid_output, valid_statistics = valid_loop(
            valid_dataloader, model, loss_fn, standarize_X, standarize_metadata
        )
        loss_valid, accuracy_valid, r2_valid = (
            valid_statistics[0],
            valid_statistics[1],
            valid_statistics[2],
        )
        ...
        draw_loss(
            fig_train, epochs_tonow, loss_epoch, accuracy_epoch, accuracy_threshold
        )
        ...
        if loss_valid <= loss_valid_min:
            ...
            print(
                "Validation loss decreased ({:.4f} --> {:.4f}).  \nSaving model ...".format(
            ...
            save_ckp(
                ...
                True,
                checkpoint_path,
                best_model_path + "_" + str(epoch + 1) + ".pt",
            )
            loss_valid_min = loss_valid
